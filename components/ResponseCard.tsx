      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6 scroll-smooth custom-scrollbar" ref={scrollRef}>
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-400 animate-in fade-in duration-700">
            <div className="w-24 h-24 rounded-full bg-white flex items-center justify-center mb-6 shadow-sm ring-4 ring-gray-100">
                 <img src={agent.avatar} alt="Agent" className="w-16 h-16 opacity-90" />
            </div>
            <p className="text-xl font-bold text-gray-800 mb-2">Hello! I'm {agent.name}.</p>
            <p className="text-sm max-w-sm text-center leading-relaxed text-gray-500">
                {agent.isTriage 
                    ? "I'm here to connect you with the right expert. Briefly describe your issue (e.g., 'billing help', 'pipeline failure')." 
