...
const App: React.FC = () => {
    ...
    try {
      ...
      if (response.handoff && response.handoff.handoff) {
          ...
          if (newAgent) {
              ...
              setIsLoading(true);
              // Updated prompt context to enforce the greeting format
              const handoffContext = `[SYSTEM: You have just been assigned to this user. The user's initial inquiry was: "${text}". Please introduce yourself, state your expertise, and end your message with "How can I help you today?". Do NOT answer the question yet, just greet them.]`;
              const specialistResponse = await geminiService.sendMessage(handoffContext, messages, [], newAgent);
  ...
  const renderContent = () => {
    switch (activeTab) {
        case 'Live Support':
            return (
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 h-[calc(100vh-140px)] animate-in fade-in duration-500">
